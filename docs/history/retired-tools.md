...
按用户确认，仅保留旧工况文件导入，不维护无消费者的旧 Python API。

- 旧 SIMPLE 独立温度/过渡区/剖面接口、非结构分区装配器、空 Protocol、
  无效 `tol_simple` 和历史 profiler 文件开关退役；当前公开模块和 F2 门槛保留。
- 无消费者报告模板、旧 HTML 补丁与固定叙事报告生成器退役；已有历史成品和
  原始 CFD/实验数据不变。非对称 CFD 工单、导入和现行研究计算继续保留。
- 上海二维/三维 frozen-Tb 循环及专属 `profile/eta/disp-c` 选项退役，
  现行上海验证调用当前求解链。按用户确认，同时退役依赖旧三维循环的
  `validate_d76_3d` 压降验证；原 D76 水侧端口未确认，不套用上海端口建立新参考。
  历史表、原 25% 压降门槛及失败状态保留原模型语境，不能由当前重跑覆盖。
  独立 `validate_sco2_d76` 六工况 Nu 验证和其 15% 门槛不受影响。
- 仍承担独立数值对照的均匀真焓、拉伸网格和编译签名检查移入 tests；它们不再
  是生产 API。旧 GUI 桥接、隐藏结果标签、偏好迁移与背景模糊移除，用户已保存
  的偏好文件不删除。

二维分区局部换热几何的缺失交接另作正确性修复。均匀工况对照不变；历史分区
结果保留原版本语境，重新计算应归属于修复后的模型，不更新旧参考数字。
