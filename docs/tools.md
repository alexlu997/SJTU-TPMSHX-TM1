...
`validate_sco2_exp_q --csv` 同样在求解前检查 CSV 及 `.meta.json` 的参考目录保护。
共享来源 CSV 写入器与该入口先在同目录暂存 CSV/元数据，全部写入成功后成组替换；
写入或替换抛出异常时保留上一组结果。此保护不覆盖断电或多个进程同时写入同一目标。
共享写入器显式设置 `sidecar=False` 时也会移除旧 sidecar，避免回读到上次运行的元数据。
