...
复现入口为 `validation/cases/validate_water_nu_excel.py`，本地详细证据位于
`.cache/water-nu-validation-20260912/`（report、summary、逐行/几何/Re 分段 CSV）。
...
见 `.cache/water-nu-refit-20260912/`。上述实测误差、10% 门槛和退出码原样保留；
...
本地对照与日志位于 `.cache/hx-experiment-decouple-20260912/`。
