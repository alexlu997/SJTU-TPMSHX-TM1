"""Partial BC + Air-Air physical regression tests (current default model).

Guards thermodynamic bounds and conservation under partial inlet/outlet BC
without the retired B-side participation experiments.
...
def test_degenerate_zero_inlet():
    """u_B=0 must not crash or divide by zero."""
    from sjtu_tpmshx.pipelines.run_stack_3d import _run_3d_stack
...
def test_full_face_B_thermodynamic_bound():
    """The full-face B boundary retains the physical thermodynamic bound."""
    from sjtu_tpmshx.pipelines.run_stack_3d import _run_3d_stack
...
# (test_epsilon_ntu_bound / test_full_face_B_thermodynamic_bound, un-xfailed). The
