...
def prepare_flow(cfg, fc, arrays, grid, side):
    ...
    K, cF = project_fields_to_streamwise_K_cF(arrays['L_field'], arrays['t_field'], cfg['tpms_type'], cfg['k_s'], ny, 0 if is_a else 3, streamwise_dx=dy, source_grid=(real_dx, real_dy) if any((cfg.get('ports_' + s) is not None for s in ('A', 'B'))) else None)
    Kfield = cFfield = None
