...
1. 启动 `sjtu_tpmshx.main`，用顶部“载入”选择预设。右侧“待计算配置”分为
   “几何／边界／求解”三页，分别填写结构与维度、流体入口与开口、网格与求解选项。
2. 点击右下角“计算”（Ctrl+R）；运行时可取消。完成后进入“结果”，选择
   温度、速度或压力，以及流体 A/B（温度还可选择固体）。三维场图可移动
   z 切片滑块；“场图 / 三维”只切换显示方式，计算维度由待计算配置决定。
   场图下方显示换热量、两侧压降、出口温度与收敛诊断；“摘要”可收起读数区，
   左侧“诊断”打开详情。编辑下一次配置不会改变当前结果的来源和单位。
   数值可显示不等于验收通过。
3. 用顶部“保存”保存配置；“导出”可保存完整 CSV + NPZ 结果或当前图像。
   相位和切片选择只改变显示及图像导出，完整结果仍保留全部原始场。
   GUI 会话保留界面状态；正式模块交接
   使用 `case.yaml` 与伴随 HDF5、`results.h5`，两者用途不同。

“更多”中可切换深色／浅色主题（重启生效）和 K／°C。中文使用微软雅黑
（Microsoft YaHei），英文和数字使用 Times New Roman，图表沿用相同字体。
字体须在本机可用；macOS 也可使用已安装 Microsoft Office 内的对应字体，
程序只在当前进程注册，不复制或分发字体。缺少字体时会在日志中提示并使用
可用系统字体；需要指定字形时请先在系统安装相应字体。
