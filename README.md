...
各流体 Nu 的适用范围另行检查。二维页面的密度循环使用 `n_rho_loops`，三维的
“最大耦合迭代次数”使用 `max_outer_3d`；内嵌页面与参数弹窗显示本次实际预算，
两维度分别保留会话内的显式编辑。未编辑时跟随当前配置默认值，编辑后以输入为准。
这些参数是计算预算，不保证收敛或实验精度。启动前只显示计划求解次数，运行中的
剩余时间估计来自实际完成情况。每次优化保存完整 `config.json`；结果的场预览
